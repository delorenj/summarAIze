...
import {getBooks, getJobs as getJobsLib, getOrCreateUser} from "./libs/user-lib";
...
export const getJobs = handler(async (event:APIGatewayProxyWithCognitoAuthorizerEvent) => {
    const userId = event.requestContext.authorizer.claims.sub;
    const jobs : ISummaryJobStatus[] = await getJobsLib(userId);
