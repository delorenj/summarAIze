...
export const chapterBreakRegex =
  /^\s*(chapter|part|section)\s(\d+|one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|thirteen|fifteen|twenty|thirty|fifty|I|V|X)*/;
export const LookForChapterHeadingParserStrategy = (
  ...
  const pageMatchWasOk = (pageMatch: any[]) => {
    const matchesWithMoreThanOneWord = pageMatch.filter(
      (match) =>
        match.trim().split(" ").length > 1 && match.trim().split(" ")[1] !== ""
    );
    console.log("matchesWithMoreThanOneWord", matchesWithMoreThanOneWord);
    console.log(
      "trim split",
      pageMatch.filter((match) => match.trim().split(" "))
    );
    if (matchesWithMoreThanOneWord.length === 0) {
      console.log(
        "Chapter break on page but it's only one word. Not counting it as a chapter"
      );
      return false;
    } else {
      console.log(
        "Chapter break on page and it's more than one word. Counting it as a chapter"
      );
      return true;
    }
  };
  const noChapterFoundOnPastXPages = (
    placeholder: IChapterPlaceholder,
    currentPage: number,
    maxNumPages: number
  ) => {
    ...
    if (result) {
      log(
        "No chapter found on past",
        ARTIFICIAL_CHAPTER_BREAK_THRESHOLD,
        "pages. Creating artificial chapter."
      );
      console.log(
        "noChapter",
        "currentPage",
        currentPage,
        "previousPage",
        placeholder.pageEnd || 0,
        "current - prev||0",
        currentPage - (placeholder.pageEnd || 0)
      );
    }
    return result;
  };
  const parse = async (doc: DocumentContext): Promise<IChapter[]> => {
    ...
    for (let i = 0; i < numPages; i++) {
      ...
      const tooLongWithoutChapter = noChapterFoundOnPastXPages(
        currentPlaceholder,
        i,
        ARTIFICIAL_CHAPTER_BREAK_THRESHOLD
      );
      if (tooLongWithoutChapter || artificialToggledOn) {
        log(
          `No chapter found on past ${ARTIFICIAL_CHAPTER_BREAK_THRESHOLD} pages, so we're going to artificially break the chapters`
        );
