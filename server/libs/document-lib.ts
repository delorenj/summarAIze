...
import {
  getBookById,
  getBookRow,
  stripNewlinesAndCollapseSpaces,
} from "./book-lib";
...
const getDescriptionFromPayload = async (
...
const getSummariesFromSummarizations = (
...
export const createSummaryDocument = async (
  summarizations: ISummarizeResult[],
  payload: ISummaryFormPayload,
  userId: string
) => {
  ...
  doc.font(titleFont).fontSize(titleSize).text(book.title, titleOptions);
  doc.moveDown(1);
  doc
  ...
  htmlData.summaries.forEach((summary) => {
    doc
      .font("Helvetica-Bold")
      .fontSize(14)
      .text("Summary of " + summary.summaryTitle, { align: "center" });
    doc.moveDown(1);
    doc
      .font("Helvetica")
      .fontSize(12)
      .text(stripNewlinesAndCollapseSpaces(summary.summaryText), {
        align: "justify",
      });
    doc.moveDown(1);
  });
...
export const uploadBlobStreamToS3 = async (
  buffer: any,
  userId: string,
  bookId: string
) => {
