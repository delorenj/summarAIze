...
const extractTextFromFileBuffer = async (fileBuffer) => {
  return new Promise((resolve, reject) => {
    fs.open(fileBuffer, 'r', (err, fd) => {
      if (err) {
        reject(err);
        return;
      }

      fs.read(fd, Buffer.alloc(fileBuffer.length), 0, fileBuffer.length, 0, (err, bytesRead, buffer) => {
        if (err) {
          reject(err);
          return;
        }
...
const getGenericMetadata = async (book) => {
  const text = await extractTextFromFileBuffer(book.fileContents);
  const textPerPage = splitStringIntoArray(text);
  console.log('textPerPage', textPerPage.length);
  const paginatedBook = {
    ...book,
    ...{textPerPage}
  };
